...
api_url = 'https://api-problem-views.dev.frstfalconi.cloud/api/v1/views'

curl_command = f'curl -X GET {api_url} -H "Content-Type: application/json"'

call_task = BashOperator(
    task_id='problem_views_dag_dev',
...
store_task = BashOperator(
    ...
    bash_command='echo {{ ti.xcom_pull(task_ids="problem_views_dag_dev") }}',
    dag=dag,
)
